...
func (t *Truncater) Perform(text string) string {
	// Returns the full text if its size is less or equal to the length passed.
	...
	// If there is no separator, it will perform the truncation by getting the
	// text truncated and appending the the omission passed.
	if t.separator == "" {
		return fmt.Sprintf("%s%s", text[:t.length-ol], t.omission)
	}

	// If the user passes a separator e.g. (" "), the function will trim the truncated
	// text and gets the the index of the last occurrence of the separator in the
	// truncated text.
	trimT := strings.Trim(text[:t.length], t.separator)
	lIndex := strings.LastIndex(trimT, t.separator)

	// If the trimT length is less than or equal to the length of the omission
	// passed, it will return the text truncated by the default value (30). Before
	// returning, it tests if the length of the raw text is not less than or equal
	// to the default value (30). If so, returns the raw text.
